...
public class Modify2 {
			...
			private static final int RANDOM_STRING_LENGTH = 6;
				...
				public static void main(String[] args) throws InterruptedException {
					...
					//driver.get("http://172.168.10.239/srm/");
			...
			Thread.sleep(5000);
				...
				for(int i=0;i<(c);i++) {
				List<WebElement> tstinput = driver.findElements(By.cssSelector(".tstnoClass.form-control"));	
				tstinput.get(i+1).sendKeys(i+adult+1+"");
				Thread.sleep(3000);
				...
				passenger2.get(i+1).click();
				/*if(adult>=1) {
				List<WebElement> passenger2 = driver.findElements(By.xpath("(//*[@title='Master. Anthony Lemiere(Child)'])"));	
				passenger2.get(i+1).click();
				}
				else {
					List<WebElement> passenger2 = driver.findElements(By.xpath("(//*[@title='Master. Anthony Lemiere(Child)'])"));	
				...
				if(adult>=1) {
					...
					for(int i=0;i<(infant);i++) {
						List<WebElement> tstinput = driver.findElements(By.cssSelector(".tstnoClass.form-control"));	
						tstinput.get(i+1).sendKeys(i+adult+child+1+"");
						Thread.sleep(3000);
